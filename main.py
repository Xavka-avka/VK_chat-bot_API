                ...
                if content[0][0] == '/':
                    if content[0][1:] == 'help':
                        ...
                        for i in (10, 130, 260, 390):
                            chest = upcoming_chests['items'][index]['name']
                            if chest == 'Tower Troop Chest':
                                session_api.messages.send(peer_id=peer_id, message="эх, какие-то новые сундуки вас ждут, когда я был сделан, их ещё не существовало, староват я", random_id=0)
                            else:
                                utils.add_chest(
                                utils.get_chests()[chest],
